...
try {
  const serviceAccount = {
    ...
    private_key: Buffer.from(process.env.FIREBASE_PRIVATE_KEY || '', 'utf8').toString().replace(/\\n/g, '\n'),    client_email: process.env.FIREBASE_CLIENT_EMAIL,
...
app.get('/message', (req, res) => {
  ...
  <script>
    ...
    async function sendMessage(){const input=document.getElementById('msgInput');const text=input.value.trim();if(!text)return;await fetch('/send-message',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({caterer,sender:currentUserName||'Customer',text})});input.value='';await loadMessages();document.getElementById('chatBox').scrollTop=document.getElementById('chatBox').scrollHeight;}
              ...
              a.onclick = function(e) {
                ...
                fetch('/caterer-read-notification', {
                  ...
                  if (text.includes('payment') || text.includes('verify')) {
                    window.location.href = '/caterer-bookings?filter=pending';
                  } else if (text.includes('booking')) {
                    window.location.href = '/caterer-bookings';
                  } else if (text.includes('message') || text.includes('chat')) {
                    window.location.href = '/caterer-messages';
                  } else {
                    window.location.href = '/caterer-dashboard';
                  }
                });
              };
...
app.post('/caterer-account/update', requireCaterer, (req, res) => { const { email, businessAddress, contactNumber, description } = req.body; const user = catererUsers.find(u => u.email === req.session.caterer.email); if (!user) return res.redirect('/caterer-login'); user.email = email; user.businessAddress = businessAddress; user.contactNumber = contactNumber; if (description !== undefined) user.description = description; req.session.caterer.email = email; const caterer = caterers.find(c => c.name === user.businessName); if (caterer) { caterer.location = businessAddress; if (description) caterer.description = description; } res.redirect('/caterer-account?tab=info&saved=1'); });
